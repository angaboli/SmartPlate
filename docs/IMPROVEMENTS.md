...
### ✅ Résolu (2026-07-21) — l'IA propose maintenant description/temps/portions manquants

**Demandé par l'utilisateur** : après le scraping, si la description, le nombre de portions, le temps de préparation ou de cuisson ne sont pas remplis, l'IA doit pouvoir les proposer plutôt que de laisser des champs vides.

**Correctif, en deux volets pour éviter tout appel IA redondant sur un même import** :
1. **Chemin Open Graph (Instagram/TikTok/YouTube)** — `structureRecipeCaption()` a été enrichie (même appel IA, même coût) : en plus de titre/ingrédients/étapes, elle propose maintenant `description`/`prepTimeMin`/`cookTimeMin`/`servings` quand la légende ne les mentionne pas explicitement, en se basant sur les ingrédients/étapes réellement trouvés. Comme pour le titre (voir la régression du 2026-07-20 plus haut), ces champs ne sont adoptés que si l'IA a trouvé une vraie structure (ingrédients/étapes non vides) — sinon la description brute reste affichée telle quelle.
2. **Chemin JSON-LD (sites de recettes)** — reste 100% déterministe par défaut (cas le plus courant, où le site fournit déjà tout). Un nouvel appel dédié, `proposeMissingRecipeFields()`, ne se déclenche que s'il manque réellement un champ (description, temps de prépa, temps de cuisson, ou portions) **et** qu'il y a de vrais ingrédients/étapes pour raisonner — ne remplace jamais un champ déjà fourni par le site source.
3. **Prompts prudents** — les deux fonctions retournent `null` plutôt que d'inventer une valeur quand rien ne permet une estimation raisonnable (ex: `cookTimeMin: null`, pas `0`, pour une salade sans cuisson ; `servings: null` si aucune quantité ne permet de déduire un nombre de portions).
4. **Vérifié avec de vrais appels OpenAI** : une légende de tabbouleh sans portions/temps de cuisson mentionnés → `cookTimeMin` et `servings` corrects à `null` (pas d'invention), `prepTimeMin` déduit du temps de trempage/repos mentionné dans les étapes ; un scénario de lasagnes façon JSON-LD incomplet → temps de cuisson proposé (45 min) qui correspond exactement à l'étape "Bake... for 45 minutes", portions réalistes (6). Vérifié aussi de bout en bout dans le vrai dialogue d'import en navigateur.
5. **Tests** : 6 nouveaux (chemin OG : champs proposés utilisés si structure trouvée / description brute + null conservés sinon ; chemin JSON-LD : ne remplace que les champs manquants, pas d'appel si tout est déjà fourni, pas d'appel sans ingrédients/étapes, pas de double appel avec le chemin OG).

**Cas limite trouvé et corrigé le jour même** — signalé par l'utilisateur avec un vrai lien (`youtube.com/shorts/a-ITrZm9xm8`) : pour cette vidéo, `shortDescription` est réellement vide (le créateur n'a pas rempli de description sur YouTube) — dans ce cas précis, YouTube affiche dans `og:description` un texte générique fixe ("Profitez des vidéos et de la musique que vous aimez...' / 'Enjoy the videos and music...") plutôt qu'un vrai extrait tronqué. Le code prenait ce texte pour une vraie description, l'envoyait à l'IA (appel gaspillé, rien à structurer) et l'affichait à l'utilisateur comme si c'était le contenu réel de la vidéo. `isGenericYouTubeDescription()` détecte ce texte connu (FR/EN) et le traite comme une absence de description : pas d'appel IA inutile, `description: null` plutôt qu'un texte trompeur, `isPartial: true` pour signaler honnêtement qu'il faut compléter à la main. Limite assumée, pas contournable par du scraping : pour un Short sans description écrite, le contenu de la recette n'existe nulle part en texte accessible (seulement dans la vidéo elle-même) — le titre reste la seule info récupérable, et c'est ce qui s'affiche. 1 nouveau test avec le cas réel exact signalé par l'utilisateur.
